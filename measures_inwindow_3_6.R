...
logfile="dax_04_logsR_v6_wind_500.txt"

n=length(y)
mas_ge=matrix(0,n-wind,2)
fileConn<-file(logfile, open="w")
#writeLines("t\tGraphEntropy\n", fileConn)
writeLines("t\tenergy\tlaplacianEnergy\testrada\tlaplacianEstrada\tspectralRadius\tglobalClusteringCoeff\n", fileConn)
close(fileConn)
...
for (i in seq(1,n-wind,by=10)) #n-wind)
{
 ...
 # 3.6
 # evb=eigenvalueBased(graphnelg,adj,2)
 #energy\tlaplacianEnergy\testrada\tlaplacianEstrada\tspectralRadius\tlocalClusteringCoeff\tglobalClusteringCoeff
 enrg=energy(graphnelg)
 lenrg=laplacianEnergy(graphnelg)
 estr=estrada(graphnelg)
 lestr=laplacianEstrada(graphnelg)
 sp_r=spectralRadius(graphnelg)

 # 3.7
# ),toString(subgr1),toString(subgr2),toString(onedDel1),toString(onedDel2),toString(twoeDel),toString(loc_clust),toString(glob_clust)
 #subrgCompl=oneEdgeDeletedSubgraphComplexity(graphnelg)
 #subgr1=subrgCompl$C_1eST
 #subgr2=subrgCompl$C_1eSpec
 #onedDelCompl=oneEdgeDeletedSubgraphComplexity(graphnelg)
 #onedDel1=onedDelCompl$C_1eST
 #onedDel2=onedDelCompl$C_1eSpec
 #twoeDel=twoEdgesDeletedSubgraphComplexity(graphnelg)
 loc_clust=localClusteringCoeff(graphnelg)
 glob_clust=globalClusteringCoeff(graphnelg)

 # 3.8
# ,toString(conID),toString(minConId),toString(prID),toString(boID),toString(balID),toString(minBalID),toString(widWid),toString(widSid),toString(huHID)
 #conID=connectivityID(graphnelg)
 #minConId=minConnectivityID(graphnelg)
 #prID=primeID(graphnelg)
 #boID=bondOrderID(graphnelg)
 #balID=balabanID(graphnelg)
 #minBalID=minBalabanID(graphnelg)
 #wid=weightedID(graphnelg)
 #widWid=wid$WID
 #widSid=wid$SID
 #huHID=huXuID(graphnelg)

 fileConn<-file(logfile, open="a")
 #writeLines(paste(toString(i),toString(enrg),toString(lenrg),toString(estr),toString(lestr),toString(sp_r),toString(subgr1),toString(subgr2),toString(onedDel1),toString(onedDel2),toString(twoeDel),toString(loc_clust),toString(glob_clust),toString(conID),toString(minConId),toString(prID),toString(boID),toString(balID),toString(minBalID),toString(widWid),toString(widSid),toString(huHID),sep="\t"), fileConn)
 writeLines(paste(toString(i),toString(enrg),toString(lenrg),toString(estr),toString(lestr),toString(sp_r),toString(glob_clust),sep="\t"), fileConn)
 close(fileConn)
